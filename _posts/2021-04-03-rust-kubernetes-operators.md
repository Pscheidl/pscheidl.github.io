...
In real world, there are many other possible states. For example, the resource might be updated by the user and changes to subresources (`Deployment` in this case) must be made. Or cases when the operator is restarted in the middle of creating the subresources required and must recover from such state. Such scenarios are omitted for simplicity, yet those do exist and real-world operator should react accordingly.
